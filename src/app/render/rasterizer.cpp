...
void Rasterizer::drawTriangles(Vector3* vertices, IShader& shader, Buffer<Uint32>* pixelBuffer, Buffer<float>* zBuffer)
{
    ...
    for (int y = yMin; y <= yMax; ++y)
    {
        for (int x = xMin; x <= xMax; ++x)
        {
            ...
            if (lambdas.data[0] < 0 || lambdas.data[1] < 0 || lambdas.data[2] < 0)
                continue;
            ...
            if ((*zBuffer)(x, y) < depth)
            {
                if (depth <= 1.0)
                {
                    (*zBuffer)(x, y) = depth;
                    (*pixelBuffer)(x, y) =
                        SDL_MapRGBA(mappingFormat, rgbVals.data[0], rgbVals.data[1], rgbVals.data[2], 0xFF);
                    ;
                }
            }
        }
...
void Rasterizer::screenSpaceTransform(Buffer<Uint32>*     pixelBuffer,
                                      Vector3*            vertices,
                                      std::array<int, 3>& xV,
                                      std::array<int, 3>& yV,
                                      Vector3&            zV)
...
void Rasterizer::baricentric(Vector3&            lambdas,
                             float               invArea,
                             int                 x,
                             int                 y,
                             std::array<int, 3>& xVerts,
                             std::array<int, 3>& yVerts)
{
    for (int i = 0; i < 3; ++i)
    {
        int i2 = (i + 1) % 3;
        lambdas.data[i] =
            invArea * ((x - xVerts[i]) * (yVerts[i2] - yVerts[i]) - (xVerts[i2] - xVerts[i]) * (y - yVerts[i]));
    }
}
