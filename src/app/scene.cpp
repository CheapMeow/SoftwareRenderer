...
Scene::Scene(const std::string& sceneName)
{
    ...
    std::string folderPath   = "../../../resources/scenes/" + sceneName;
...
std::queue<Model*>* Scene::getVisiblemodels() { return &visibleModels; }

BaseLight* Scene::getCurrentLights() { return lights; }
                ...
                while (!file.eof())
                {
                    ...
                    { // model related setup
                        ...
                        for (int i = 0; i < max; ++i)
                        {
                            ...
                            rot >> key >> x >> y >> z;
                            initParameters.rotation =
                                Vector3f(stof(x) * M_PI / 180.0f, stof(y) * M_PI / 180.0f, stof(z) * M_PI / 180.0f);
...
int Scene::getLightCount() { return lightCount; }
