    ...
    for (point.y = yMin; point.y <= yMax; ++point.y)
    {
        ...
        for (point.x = xMin; point.x <= xMax; ++point.x)
        {
            ...
            if (inside(e.x, A01, B01) && inside(e.y, A12, B12) && inside(e.z, A20, B20))
            {
                Vector3f tmp = e * area;
                // Run fragment shader
                shader.fragment(tmp, rgbVals, depth, zVals);
                // Zbuffer check
...
void Rasterizer::triBoundBox(int&            xMax,
                             int&            xMin,
                             int&            yMax,
                             int&            yMin,
                             Vector3f*       vertices,
...
{
    xMax = std::ceil(std::max({
        vertices[0].x,
        vertices[1].x,
        vertices[2].x,
    }));
    xMin = std::ceil(std::min({
        vertices[0].x,
        vertices[1].x,
        vertices[2].x,
    }));
    ...
    yMax = std::ceil(std::max({
        vertices[0].y,
        vertices[1].y,
        vertices[2].y,
    }));
    yMin = std::ceil(std::min({
        vertices[0].y,
        vertices[1].y,
        vertices[2].y,
    }));
