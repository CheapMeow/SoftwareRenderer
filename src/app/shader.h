...
{
    virtual ~IShader() {};
    virtual Vector3f vertex(const Vector3f& vertex,
                            const Vector3f& normal,
                            const Vector3f& textureVals,
...
struct FlatShader : public IShader
{
    ...
    Vector3f vertex(const Vector3f& vertex,
                    const Vector3f& normal,
                    const Vector3f& textureVals,
                    const Vector3f& tangent,
                    const Vector3f& light,
    ...
    Vector3f fragment(float u, float v) override { return rgb * varIntensity; }
};
...
struct GouraudShader : public IShader
{
    ...
    Vector3f fragment(float u, float v) override
    {
        ...
        diffStrength = varying_diffuse.x + u * (varying_diffuse.y - varying_diffuse.x) +
                       v * (varying_diffuse.z - varying_diffuse.x);
        diffuse = lightColor2 * diffStrength;

        spec = varying_specular.x + u * (varying_specular.y - varying_specular.x) +
               v * (varying_specular.z - varying_specular.x);
...
struct TextureMapShader : public IShader
{
    ...
    Vector3f interpCoords, interpLightDir, interpNormal, interpViewDir, interpCol, interpAO;
...
struct PBRShader : public IShader
{
    ...
    // Interpolated variables
    Vector3f interpCoords, interpLightDir, interpNormal, interpViewDir, interpCol;
    ...
    Vector3f fresnelSchlick(float cosTheta, Vector3f& fresnel0)
    {
        ...
        return fresnel0 +
               (Vector3f(1.0) - fresnel0) * (invcCosTheta * invcCosTheta * invcCosTheta * invcCosTheta * invcCosTheta);
    }
    ...
    Vector3f fragment(float u, float v) override
    {
        ...
        Vector3f* radiaceLights = (Vector3f*)malloc(maxLights * sizeof(Vector3f));
        // #pragma omp simd
        for (int light = 0; light < maxLights; ++light)
        {
            int val        = light * 3;
            interpLightDir = lightDirVal[val] + (lightDirVal[val + 1] - lightDirVal[val]) * u +
                             (lightDirVal[val + 2] - lightDirVal[val]) * v;
