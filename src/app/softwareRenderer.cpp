...
void SoftwareRenderer::drawTriangularMesh(Model* currentModel)
{
    ...
    Vector3f trianglePrimitive[3], normalPrim[3], uvPrim[3], tangentPrim[3];
...
#pragma omp parallel for private(trianglePrimitive, normalPrim, uvPrim, tangentPrim) firstprivate(shader)
    ...
    for (int j = 0; j < numFaces; ++j)
    {
        ...
        if (backFaceCulling((*fNormals)[j], trianglePrimitive[0], worldToObject))
            continue;
        ++count;
        // Apply vertex shader
        for (int i = 0; i < 3; ++i)
        {
            trianglePrimitive[i] =
                shader.vertex(trianglePrimitive[i], normalPrim[i], uvPrim[i], tangentPrim[i], dummyDir, i);
        }

        // Skip triangles that are outside viewing frustrum
        // Does not rebuild triangles that are partially out TO DO
        if (clipTriangles(trianglePrimitive))
            continue;
...
void SoftwareRenderer::setCameraToRenderFrom(Camera* camera) { mCamera = camera; }
...
bool SoftwareRenderer::clipTriangles(Vector3f* clipSpaceVertices)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        bool     inside =
            (-vertex.w <= vertex.x <= vertex.w) && (-vertex.w <= vertex.y <= vertex.w) && (0 <= vertex.z <= vertex.w);
        if (!inside)
            ++count;
    }
